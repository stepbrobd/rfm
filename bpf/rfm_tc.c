...
static __always_inline int rfm_tc(struct __sk_buff *skb, __u8 dir) {
  __u32 zero = 0;
  struct rfm_config *cfg = bpf_map_lookup_elem(&rfm_config, &zero);
  if (!cfg || cfg->sample_rate == 0)
    return TC_ACT_OK;

  // parse ethernet header for L3 protocol
  ...
  // update iface stats (always, not gated by sampling)
  ...
  struct rfm_iface_value *val = bpf_map_lookup_elem(&rfm_iface_stats, &ikey);
  if (val) {
    val->packets++;
    val->bytes += skb->len;
  } else {
    struct rfm_iface_value init = {.packets = 1, .bytes = skb->len};
...
SEC("tc/egress")
int rfm_tc_egress(struct __sk_buff *skb) { return rfm_tc(skb, RFM_DIR_EGRESS); }
